...
while True:
    vendedor = input("informe o nome do vendedor: ").casefold()
    if "sair" in vendedor:
        break
    else: 
        venda_produto = float(input("Informe a quantidade de vendas do produto especificado: "))
        if vendedor not in vendedores:
            vendedores[vendedor] = [venda_produto]
        else:
            vendedores[vendedor].append(venda_produto)



for key,values in vendedores.items():
    total_vendas =  sum(values)
    media_vendas = total_vendas / len(values)
    print(f"o Produto: {key}, tem um valor total de: {total_vendas:.2f}, e uma media de: {media_vendas:.2f}")
